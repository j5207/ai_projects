...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: 
            # Set probability for jail position to 1.0
            # Other cells will have a default value of 0.0
            allPossible[self.getJailPosition()] = 1.0 
        else:
            for p in self.legalPositions:
                trueDistance = util.manhattanDistance(p, pacmanPosition)
                if emissionModel[trueDistance] > 0:
                    # Belief update = P(e|x) * P(x)
                    allPossible[p] =\
                            emissionModel[trueDistance] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # Time elapse: P(x') = sum(x){P(x'|x) * P(x)}
        for oldPos in self.legalPositions: # For every possible x
            newPosDist = self.getPositionDistribution(
                            self.setGhostPosition(gameState, oldPos) )
            for newPos, prob in newPosDist.items():
                # P(x') += P(x'|x) * P(x)
                newBelief[newPos] += prob * oldBelief[oldPos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        numParticles = self.numParticles # number of particles to initialize
        legalPositions = self.legalPositions
        particles = []

        # If the number of particles is more than legal positions,
        # there can be more than one particle in each position.
        # Initialize one particle for each position first.
        while numParticles > len(legalPositions):
            numParticles -= len(legalPositions)
            for position in legalPositions: particles.append(position) 

        # randomize the legal positions for particle insertion
        random.shuffle(legalPositions) 
        for position in legalPositions:
            # Initialize one particle for each position.
            # Since legalPositions is shuffled, the distribution is uniform 
            # and random
            particles.append(position)
            numParticles -= 1
            if not numParticles: break # No more particle to initialize

        self.particles = particles # Store the initialized particles
    ...
    def observe(self, observation, gameState):
        ...
        pacmanPosition = gameState.getPacmanPosition()
        "*** YOUR CODE HERE ***"
        if noisyDistance == None: # The ghost is captured
            # Set all particles to jail
            self.particles = [self.getJailPosition()] * self.numParticles
        else:
            # Calculate the weights of particles based on observation
            weights = util.Counter()
            for p in self.particles:
                distance = util.manhattanDistance(pacmanPosition, p)
                if emissionModel[distance] > 0:
                    # Use += operation instead of = to account for multiple
                    # particles in the same location
                    weights[p] += emissionModel[distance]

            # Normalize the weights of particles
            weights.normalize()

            if weights.totalCount() == 0:
                # All particles receive 0 weight: reinitialize particles
                self.initializeUniformly(gameState)
            else:
                # Resample particles base on weight
                self.particles = \
                    [util.sample(weights) for _ in xrange(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.particles: # For every particle
            ...
            # Select a new position based on the distribution
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # If number of particles is more than the number of permutations,
        # then there can be more than one particle representing the same
        # permutation. Initialize one particle for each permutation first
        while numParticles > len(candidates):
            numParticles -= len(candidates)
            self.particles += candidates

        # Randomize the permutations and select the first {numParticles}
        # candidates as particles. This way, the final distribution is 
        # uniform and random
        random.shuffle(candidates)
        self.particles += candidates[0:numParticles-1]
    ...
    def observeState(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        # Detect and modify dead ghost particles
        #for i in xrange(self.numGhosts):
        #    if noisyDistances[i] is None: # This ghost is captured
        #        for p in self.particles: 
        #            # Put this ghost in jail for all particles
        #            p = self.getParticleWithGhostInJail(p, i)

        # Update weights based on observations
        weights = util.Counter()
        for p in self.particles:
            particleProbability = 1.0
            for i in xrange(self.numGhosts):
                if noisyDistances[i] == None: # This ghost is captured
                    # Send this ghost to jail for this particle
                    p = self.getParticleWithGhostInJail(p, i)
                else:
                    distance = util.manhattanDistance(pacmanPosition, p[i])
                    particleProbability *= emissionModels[i][distance]
            weights[p] += particleProbability
        weights.normalize()
